...
export default function DiscountCodesAdminPanel({
  ...
  const loadLinkStats = useCallback(async () => {
    ...
    try {
      ...
      console.error(err);
      // Never blank the catalog (Arabella, Pedro, …) if the live list fails.
      const fallback = [...VALID_DISCOUNT_CODES];
      const next: Record<string, DiscountLinkMeta> = {};
      for (const code of fallback) {
        next[code] = { code, clicks: 0, autoApplyUrl: fallbackAutoApplyUrl(code) };
      }
      setLinkMetaByCode(next);
      setActiveCodes(fallback);
    } finally {
        ...
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-neutral-500">
              Creator codes and short links stay live. Approving a new affiliate
              does not remove Arabella, Aryan, Pedro, or the others. Click a code
              to copy its influencer short link.
            </p>
          </div>
          ...
          <div className="space-y-3 border-t border-black/5 pt-3">
            <div className="rounded-xl border border-amber-200 bg-amber-50/80 px-4 py-3 space-y-2">
              ...
              <div className="flex flex-col sm:flex-row gap-2">
                ...
                {selectedGroup.source === "affiliate" &&
                !isValidDiscountCode(selectedGroup.code) ? (
                  <button
                    type="button"
          ...
          <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3 border-t border-black/5 pt-3">
            ...
            {groups.map((group) => (
              ...
              >
                ...
                {group.source === "affiliate" && !isValidDiscountCode(group.code) ? (
